...
import {
  ChevronLeft, ArrowUpRight, ArrowDownLeft, Smartphone,
  Receipt, RefreshCw, Eye, EyeOff, Sparkles, Clock,
...
type Txn = { id: string; label: string; amount: number; direction: 'in' | 'out'; ts: number; status: 'done' | 'pending' | 'failed'; provider: Provider };

const SAMPLE_TXNS: Txn[] = [
  { id: 't1', label: 'Uza Mahindi — Mwanzo Coop',  amount: 84000,  direction: 'in',  ts: Date.now() - 3600000,    status: 'done',    provider: 'mpesa'  },
  { id: 't2', label: 'Mbolea — Yara Tanzania',      amount: 42500,  direction: 'out', ts: Date.now() - 7200000,    status: 'done',    provider: 'mpesa'  },
  { id: 't3', label: 'Ada ya Kilimo — TARI',        amount: 5000,   direction: 'out', ts: Date.now() - 86400000,   status: 'done',    provider: 'airtel' },
  { id: 't4', label: 'Pato la Mahindi — Sehemu 2', amount: 56000,  direction: 'in',  ts: Date.now() - 172800000,  status: 'done',    provider: 'mpesa'  },
  { id: 't5', label: 'Airtime — Vodacom',           amount: 3000,   direction: 'out', ts: Date.now() - 259200000,  status: 'done',    provider: 'airtel' },
  { id: 't6', label: 'Usafirishaji wa Mazao',       amount: 12000,  direction: 'out', ts: Date.now() - 345600000,  status: 'pending', provider: 'mpesa'  },
];
...
const pt = StyleSheet.create({ tag: { paddingHorizontal: 6, paddingVertical: 2, borderRadius: 4 }, txt: { fontFamily: 'Inter_800ExtraBold', fontSize: 8 } });
...
export default function MobileMoneyScreen() {
  ...
  return (
    <View style={[s.root, { backgroundColor: colors.background }]}>
      <LinearGradient
        colors={isDark ? ['#060a04', '#080e05', colors.background] : ['#f0fdf4', '#f8fafc', colors.background]}
      ...
      <SafeAreaView style={{ flex: 1 }}>
        ...
        <View style={s.header}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={[s.title, { color: colors.text }]}>{language === 'sw' ? 'Pesa za Simu' : 'Mobile Money'}</Text>
          </View>
          <View style={s.providerRow}>
            {(['mpesa', 'airtel'] as Provider[]).map(p => (
              <TouchableOpacity key={p} onPress={() => { Haptics.selectionAsync(); setProvider(p); }}
                style={[s.provBtn, {
                  backgroundColor: provider === p ? (p === 'mpesa' ? '#00a54f' : '#ef4444') : colors.card,
        ...
        <ScrollView contentContainerStyle={{ paddingBottom: 40 }} showsVerticalScrollIndicator={false}>
            ...
            >
              ...
              <View style={s.balFooter}>
                <View style={[s.balBadge, { backgroundColor: 'rgba(255,255,255,0.15)' }]}>
                  ...
                  <Text style={s.balBadgeTxt}>{language === 'sw' ? 'Akaunti Hai' : 'Active Account'}</Text>
                </View>
                <TouchableOpacity onPress={() => { Haptics.selectionAsync(); }} style={[s.balBadge, { backgroundColor: 'rgba(255,255,255,0.15)' }]}>
          ...
          <View style={s.section}>
            <Text style={[s.sectionTitle, { color: colors.textMute }]}>{language === 'sw' ? 'VITENDO VYA HARAKA' : 'QUICK ACTIONS'}</Text>
            <View style={s.actionsGrid}>
              {QUICK_ACTIONS.map((a, i) => (
                <Animated.View key={a.id} entering={FadeInDown.delay(i * 60).springify()} style={{ flex: 1 }}>
                  <TouchableOpacity
                  ...
                  >
                    <View style={[s.actionIcon, { backgroundColor: activeAction === a.id ? 'rgba(255,255,255,0.2)' : a.color + '18' }]}>
                      <a.icon size={20} color={activeAction === a.id ? '#fff' : a.color} />
                    </View>
                    <Text style={[s.actionLabel, { color: activeAction === a.id ? '#fff' : colors.text }]}>
            ...
            <Animated.View entering={FadeInDown.springify()} style={{ paddingHorizontal: 16, marginBottom: 8 }}>
              <View style={[s.formCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
                <Text style={[s.formTitle, { color: colors.text }]}>
                  {activeAction === 'send' ? (language === 'sw' ? 'Tuma Pesa' : 'Send Money') :
                   activeAction === 'receive' ? (language === 'sw' ? 'Omba Pesa' : 'Request Money') :
                   (language === 'sw' ? 'Lipa Bili' : 'Pay Bill')}
                </Text>
                <TextInput
                  style={[s.input, { backgroundColor: colors.background, borderColor: colors.border, color: colors.text }]}
                  placeholder={language === 'sw' ? 'Nambari ya simu (e.g. 0712...)' : 'Phone number (e.g. 0712...)'}
                ...
                <TouchableOpacity onPress={handleSubmit}
                  style={[s.submitBtn, { backgroundColor: provider === 'mpesa' ? '#00a54f' : '#ef4444' }]}>
          ...
          <View style={s.section}>
            <View style={s.sectionRow}>
              <Text style={[s.sectionTitle, { color: colors.textMute }]}>{language === 'sw' ? 'MIAMALA YA HIVI KARIBUNI' : 'RECENT TRANSACTIONS'}</Text>
              <TouchableOpacity onPress={() => router.push('/wallet-admin/transactions' as any)}>
                <Text style={[s.seeAll, { color: colors.primary }]}>{language === 'sw' ? 'Zote →' : 'See all →'}</Text>
              </TouchableOpacity>
            </View>
                ...
                <Animated.View key={t.id} entering={FadeInDown.delay(i * 40).springify()}>
                  <View style={[s.txnRow, { backgroundColor: colors.card, borderColor: colors.border }]}>
                    ...
                    <View style={{ flex: 1, gap: 2 }}>
                      <Text style={[s.txnLabel, { color: colors.text }]} numberOfLines={1}>{t.label}</Text>
                      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                        <ProviderTag p={t.provider} />
                        <Clock size={10} color={colors.textMute} />
                        <Text style={[s.txnMeta, { color: colors.textMute }]}>{fmtAge(t.ts)} {language === 'sw' ? 'iliyopita' : 'ago'}</Text>
          ...
          {/* Link to full wallet */}
          <Animated.View entering={FadeInDown.delay(300).springify()} style={{ paddingHorizontal: 16 }}>
            <TouchableOpacity onPress={() => router.push('/wallet-admin' as any)}
...
const s = StyleSheet.create({
  ...
  balCard:      { borderRadius: 20, padding: 20, marginBottom: 8, shadowOffset: { width: 0, height: 8 }, shadowOpacity: 0.3, shadowRadius: 16, elevation: 8 },
  ...
  balProvider:  { fontFamily: 'Inter_800ExtraBold', fontSize: 10, color: 'rgba(255,255,255,0.5)', letterSpacing: 1, marginTop: 2 },
  eyeBtn:       { padding: 4 },
  balAmount:    { fontFamily: 'InstrumentSerif_400Regular', fontSize: 36, color: '#fff', marginVertical: 12 },
  balFooter:    { flexDirection: 'row', gap: 8 },
  balBadge:     { flexDirection: 'row', alignItems: 'center', gap: 5, paddingHorizontal: 10, paddingVertical: 5, borderRadius: 999 },
  ...
  sectionTitle: { fontFamily: 'Inter_800ExtraBold', fontSize: 10, letterSpacing: 0.8, paddingHorizontal: 16, marginBottom: 10 },
  ...
  input:        { height: 48, borderRadius: 12, borderWidth: 1, paddingHorizontal: 14, fontFamily: 'Inter_500Medium', fontSize: 14 },
  ...
  txnRow:       { flexDirection: 'row', alignItems: 'center', padding: 12, borderRadius: 14, borderWidth: 1, gap: 12 },
  txnDot:       { width: 38, height: 38, borderRadius: 19, alignItems: 'center', justifyContent: 'center' },
  ...
  walletLink:   { flexDirection: 'row', alignItems: 'center', gap: 12, padding: 16, borderRadius: 16, borderWidth: 1, marginTop: 8 },
