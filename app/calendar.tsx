...
import Animated, {
  FadeInDown, FadeIn, FadeInUp, SlideInUp,
...
import {
  ChevronLeft, ChevronRight, Plus, X, Check, Sparkles,
  BrainCircuit, Droplets, Leaf, Target, BarChart3, Microscope,
  ArrowRight, Send, CheckCircle2, Calendar, Flag, Clock,
  Trash2, LayoutGrid, MessageSquare, AlertCircle, RefreshCw
...
const PRIORITY_COLOR: Record<string, string> = {
  critical: '#ef4444', high: '#f59e0b', medium: '#22d15a', low: '#94a3b8',
};
const PRIORITY_LABEL_SW: Record<string, string> = {
  critical: 'DHARURA', high: 'JUU', medium: 'KATI', low: 'CHINI',
};
...
const PRIORITY_WORDS: Record<string, TaskPriority> = {
  ...
  haraka: 'high', high: 'high', juu: 'high',
  kawaida: 'medium', normal: 'medium', medium: 'medium',
  baadaye: 'low', low: 'low', chini: 'low',
};
const DAY_MAP: Record<string, number> = {
  sunday: 0, jumapili: 0,
  monday: 1, jumatatu: 1,
  tuesday: 2, jumanne: 2,
  wednesday: 3, jumatano: 3,
  thursday: 4, alhamisi: 4,
  friday: 5, ijumaa: 5,
  saturday: 6, jumamosi: 6,
};

interface ParsedTask { title: string; date: Date; priority: TaskPriority; category: TaskCategory }
...
function parseNLTask(input: string): ParsedTask | null {
  ...
  for (const [word, p] of Object.entries(PRIORITY_WORDS)) {
    if (lower.includes(word)) { priority = p; break; }
  }
...
const CatIcon = ({ cat, size, color }: { cat: string; size: number; color: string }) => {
  switch (cat) {
    case 'irrigation': return <Droplets size={size} color={color} />;
    case 'planting':   return <Leaf size={size} color={color} />;
    case 'harvest':    return <Sparkles size={size} color={color} />;
    case 'scouting':   return <Microscope size={size} color={color} />;
    case 'finance':    return <BarChart3 size={size} color={color} />;
...
const CompletionBurst = ({ onDone }: { onDone: () => void }) => {
  ...
  return (
    <Animated.View style={[StyleSheet.absoluteFill, { alignItems: 'center', justifyContent: 'center', zIndex: 99 }, style]}>
      <View style={{ backgroundColor: '#22d15a', borderRadius: 20, padding: 10 }}>
...
export default function CalendarScreen() {
  ...
  const dayTasks = useMemo(() => {
    ...
    if (filter === 'deadlines') return all.filter(t => t.priority === 'critical' || t.priority === 'high');
  ...
  // ── AI handler ───────────────────────────────────────────────────────────────
  const handleAI = useCallback(async (queryOverride?: string) => {
    ...
    try {
      ...
      if (isBulk) {
        ...
        const todayPending = tasks.filter(t =>
          t.dueDate && toDateKey(new Date(t.dueDate)) === todayKey &&
...
If suggesting task changes, be specific. Max 3 sentences.`;
        ...
        // Offline fallback
        const overloaded = Object.entries(tasksByKey)
          .filter(([, ts]) => ts.filter(t => t.status !== 'done').length > 2)
          .length;
        replyText = language === 'sw'
          ? overloaded > 0
      ...
      setAIHistory(h => [...h, { role: 'ai', text: language === 'sw' ? 'Kuna tatizo. Jaribu tena.' : 'Something went wrong. Try again.' }]);
    } finally {
      setAILoading(false);
      setTimeout(() => aiScrollRef.current?.scrollToEnd({ animated: true }), 150);
    }
  }, [aiInput, tasks, dayTasks, tasksByKey, pendingCount, curMonth, curYear, selected, today, language, createTask, cancelTask]);
  ...
  const renderGrid = () => {
    ...
    return (
      <View style={[styles.gridCard, { backgroundColor: isDark ? 'rgba(6,14,8,0.98)' : colors.card, borderColor: isDark ? 'rgba(255,255,255,0.06)' : colors.border }]}>
        <LinearGradient colors={['rgba(34,209,90,0.07)','transparent']} start={{x:0,y:0}} end={{x:1,y:1}} style={StyleSheet.absoluteFill} pointerEvents="none" />
          ...
          {calGrid.map((day, i) => {
            ...
            return (
              ...
              >
                <Text style={[
                  styles.cellNum,
                  { color: isSel ? '#000' : isToday ? '#22d15a' : isPast ? colors.textMute + '60' : isDark ? '#fff' : colors.text },
                ...
                {dots.length > 0 && (
                  <View style={styles.dotRow}>
                    {dots.map((t, di) => (
                      <View key={di} style={[styles.taskDot, { backgroundColor: PRIORITY_COLOR[t.priority] || '#22d15a' }]} />
                    ))}
                  </View>
                )}
              </TouchableOpacity>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.filterRow}>
        {filters.map(f => (
          <TouchableOpacity
            ...
            style={[
              styles.filterPill,
              filter === f.id && { backgroundColor: '#22d15a', borderColor: '#22d15a' },
          ...
          >
            <Text style={[styles.filterText, { color: filter === f.id ? '#000' : colors.textMute }]}>
        ...
        <View style={styles.dayHeader}>
          <View>
            ...
            <Text style={[styles.dayHeaderSub, { color: colors.textMute }]}>
              {dayTasks.length} {language === 'sw' ? 'kazi' : 'tasks'} · {dayTasks.filter(t => t.status === 'done').length} {language === 'sw' ? 'zilizokamilika' : 'done'}
            </Text>
          </View>
          <TouchableOpacity
            onPress={() => openAdd()}
            style={[styles.addDayBtn, { backgroundColor: 'rgba(34,209,90,0.12)', borderColor: 'rgba(34,209,90,0.25)' }]}
          >
        ...
        {dayTasks.length === 0 ? (
          <Animated.View entering={FadeIn} style={styles.emptyDay}>
            ...
            >
              ...
              <Text style={styles.emptyDayBtnText}>{language === 'sw' ? 'Ongeza Kazi' : 'Add a Task'}</Text>
            </TouchableOpacity>
          </Animated.View>
        ) : (
          <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ gap: 8, paddingBottom: 120 }}>
            {dayTasks.map((task, idx) => {
              const pc = PRIORITY_COLOR[task.priority] || '#22d15a';
              ...
              return (
                <Animated.View key={task.id} entering={FadeInDown.delay(idx * 40).springify()}>
                  <View style={[styles.taskRow, {
                    backgroundColor: isDark ? (isDone ? 'rgba(255,255,255,0.02)' : 'rgba(255,255,255,0.04)') : (isDone ? 'rgba(0,0,0,0.02)' : colors.card),
                    borderColor: isDone ? 'transparent' : (isDark ? 'rgba(255,255,255,0.07)' : colors.border),
                    ...
                    <View style={{ flex: 1 }}>
                      ...
                      <View style={{ flexDirection: 'row', gap: 6, marginTop: 4, alignItems: 'center' }}>
                        <View style={[styles.priBadge, { backgroundColor: pc + '18', borderColor: pc + '35' }]}>
                          <Text style={[styles.priBadgeText, { color: pc }]}>
                            {language === 'sw' ? PRIORITY_LABEL_SW[task.priority] : PRIORITY_LABEL_EN[task.priority]}
                          </Text>
                        </View>
                        {task.farmBlock && (
                          <Text style={[styles.taskBlock, { color: colors.textMute }]}>{task.farmBlock}</Text>
                        )}
                      </View>
                    </View>
                    ...
                    <View style={{ gap: 6 }}>
                      {!isDone && (
                        <TouchableOpacity
                          ...
                          style={[styles.completeBtn, { backgroundColor: 'rgba(34,209,90,0.12)', borderColor: 'rgba(34,209,90,0.25)' }]}
                        >
                          <Check size={13} color="#22d15a" strokeWidth={2.5} />
                        </TouchableOpacity>
                      )}
                      <TouchableOpacity
                        onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); cancelTask(task.id); }}
      ...
      <View style={styles.modalOverlay}>
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
          <Animated.View entering={SlideInUp.springify()} style={[styles.modalSheet, { backgroundColor: isDark ? '#0a130b' : colors.card, borderColor: isDark ? 'rgba(255,255,255,0.08)' : colors.border }]}>
            <LinearGradient colors={['rgba(34,209,90,0.08)','transparent']} start={{x:0,y:0}} end={{x:1,y:1}} style={StyleSheet.absoluteFill} pointerEvents="none" />
            ...
            <View style={styles.modalHeader}>
              <View>
                ...
                <Text style={[styles.modalSub, { color: colors.textMute }]}>
                  {newDate.getDate()} {(language === 'sw' ? MONTHS_SW : MONTHS_EN)[newDate.getMonth()]} {newDate.getFullYear()}
                </Text>
              </View>
              <TouchableOpacity onPress={() => setShowAdd(false)} style={[styles.closeBtn, { borderColor: isDark ? 'rgba(255,255,255,0.1)' : colors.border }]}>
                <X size={18} color={colors.text} />
              </TouchableOpacity>
            </View>

            {/* Title input */}
            <View style={[styles.inputWrap, { backgroundColor: isDark ? 'rgba(255,255,255,0.05)' : colors.background, borderColor: isDark ? 'rgba(255,255,255,0.1)' : colors.border }]}>
              <TextInput
            ...
            <View style={styles.priorityRow}>
              {(['low','medium','high','critical'] as TaskPriority[]).map(p => (
                ...
                >
                  <Text style={[styles.priBtnText, { color: newPriority === p ? '#000' : PRIORITY_COLOR[p] }]}>
              ...
              <View style={{ flexDirection: 'row', gap: 8 }}>
                {CATEGORIES.map(c => (
                  <TouchableOpacity
                    ...
                    style={[styles.catBtn, {
                      backgroundColor: newCategory === c.id ? 'rgba(34,209,90,0.15)' : 'transparent',
                      borderColor: newCategory === c.id ? '#22d15a' : (isDark ? 'rgba(255,255,255,0.1)' : colors.border),
                    }]}
                  >
                    <CatIcon cat={c.id} size={14} color={newCategory === c.id ? '#22d15a' : colors.textMute} />
                    <Text style={[styles.catBtnText, { color: newCategory === c.id ? '#22d15a' : colors.textMute }]}>
            ...
            <TouchableOpacity
              ...
              style={[styles.aiHint, { backgroundColor: 'rgba(34,209,90,0.07)', borderColor: 'rgba(34,209,90,0.2)' }]}
            >
              <BrainCircuit size={13} color="#22d15a" />
              <Text style={[styles.aiHintText, { color: '#22d15a' }]}>
            ...
            <View style={styles.modalActions}>
              <TouchableOpacity onPress={() => setShowAdd(false)} style={[styles.cancelBtn, { borderColor: isDark ? 'rgba(255,255,255,0.1)' : colors.border }]}>
                <Text style={[styles.cancelBtnText, { color: colors.textMute }]}>{language === 'sw' ? 'Futa' : 'Cancel'}</Text>
              </TouchableOpacity>
              <TouchableOpacity
                onPress={handleSaveTask}
                disabled={!newTitle.trim() || saving}
                style={[styles.saveBtn, { backgroundColor: newTitle.trim() ? '#22d15a' : colors.textMute + '40' }]}
              >
  ...
  const QUICK_PROMPTS = language === 'sw'
    ? ['Ongeza kazi leo', 'Songa kazi zote leo kesho', 'Nionyeshe kazi nyingi', 'Jinsi ya kutumia kalenda?']
      ...
      <View style={styles.aiOverlay}>
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
          <Animated.View entering={SlideInUp.springify()} style={[styles.aiSheet, { backgroundColor: isDark ? '#08110a' : colors.card, borderColor: isDark ? 'rgba(255,255,255,0.08)' : colors.border }]}>
            <LinearGradient colors={['rgba(34,209,90,0.1)','rgba(34,209,90,0.03)','transparent']} start={{x:0,y:0}} end={{x:1,y:1}} style={StyleSheet.absoluteFill} pointerEvents="none" />
            ...
            <View style={styles.aiHeader}>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
                ...
                <View style={[styles.aiLiveDot, { backgroundColor: 'rgba(34,209,90,0.15)', borderColor: 'rgba(34,209,90,0.3)' }]}>
                  <View style={{ width: 5, height: 5, borderRadius: 3, backgroundColor: '#22d15a' }} />
                  <Text style={{ fontSize: 8, fontFamily: 'Inter_700Bold', color: '#22d15a' }}>LIVE</Text>
                </View>
              </View>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.quickPrompts}>
              {QUICK_PROMPTS.map((p, i) => (
                <TouchableOpacity key={i} onPress={() => handleAI(p)} style={[styles.quickPill, { backgroundColor: isDark ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.04)', borderColor: isDark ? 'rgba(255,255,255,0.08)' : colors.border }]}>
            ...
            >
              {aiHistory.map((msg, i) => (
                <Animated.View key={i} entering={FadeInDown.delay(i === aiHistory.length - 1 ? 0 : 0).springify()}>
                  <View style={[
                    styles.chatBubble,
                    msg.role === 'user'
                      ? [styles.chatUser, { backgroundColor: '#22d15a' }]
            ...
            {/* Input row */}
            <View style={[styles.aiInputRow, { borderColor: isDark ? 'rgba(255,255,255,0.1)' : colors.border, backgroundColor: isDark ? 'rgba(255,255,255,0.04)' : colors.background }]}>
              <TextInput
              ...
              <TouchableOpacity
                ...
                style={[styles.aiSendBtn, { backgroundColor: aiInput.trim() ? '#22d15a' : 'transparent' }]}
              >
                {aiLoading ? <ActivityIndicator size="small" color="#22d15a" /> : <Send size={16} color={aiInput.trim() ? '#000' : colors.textMute} />}
              </TouchableOpacity>
            </View>
          </Animated.View>
        </KeyboardAvoidingView>
  ...
  return (
    <View style={[styles.root, { backgroundColor: colors.background }]}>
      ...
      <SafeAreaView style={{ flex: 1 }}>
        ...
        <View style={styles.header}>
          ...
          <TouchableOpacity
            ...
            style={[styles.aiHeaderBtn, { backgroundColor: 'rgba(34,209,90,0.12)', borderColor: 'rgba(34,209,90,0.25)' }]}
          >
          ...
          <TouchableOpacity
            onPress={() => openAdd()}
            style={[styles.addHeaderBtn, { backgroundColor: '#22d15a' }]}
          >
      ...
      <TouchableOpacity
        onPress={() => openAdd()}
        style={[styles.fab, { backgroundColor: '#22d15a' }]}
        activeOpacity={0.85}
      >
...
const styles = StyleSheet.create({
  ...
  header: {
    ...
    paddingTop: Platform.OS === 'android' ? 16 : 4, paddingBottom: 10, gap: 10,
  },
  ...
  aiHeaderBtnText: { fontSize: 11, fontFamily: 'Inter_700Bold', color: '#22d15a' },
  addHeaderBtn: { width: 34, height: 34, borderRadius: 17, alignItems: 'center', justifyContent: 'center' },
  ...
  gridCard: {
    borderRadius: 20, borderWidth: 1, overflow: 'hidden', padding: 14, marginBottom: 10,
  },
  ...
  cellSelected: { backgroundColor: '#22d15a' },
  cellToday:    { borderWidth: 1.5, borderColor: '#22d15a', borderRadius: 10 },
  ...
  addDayBtnText: { fontSize: 11, fontFamily: 'Inter_700Bold', color: '#22d15a' },
  ...
  taskRow: {
    ...
    borderRadius: 14, borderWidth: 1, overflow: 'hidden', position: 'relative',
  },
  ...
  deleteBtn: {
    width: 26, height: 26, borderRadius: 13, alignItems: 'center', justifyContent: 'center', borderWidth: 1,
  },

  fab: {
    position: 'absolute', bottom: 28, right: 24, width: 56, height: 56,
    borderRadius: 28, alignItems: 'center', justifyContent: 'center',
    shadowColor: '#22d15a', shadowOffset: { width: 0, height: 4 },
  ...
  modalSheet: {
    ...
    borderLeftWidth: 1, borderRightWidth: 1, padding: 20, overflow: 'hidden',
  },
  modalHeader: {
  ...
  inputWrap: { borderRadius: 14, borderWidth: 1, paddingHorizontal: 12, paddingVertical: 10, marginBottom: 16 },
  ...
  aiAvatar: {
    ...
    backgroundColor: 'rgba(34,209,90,0.15)', borderWidth: 1, borderColor: 'rgba(34,209,90,0.3)',
  },
  ...
  aiLiveDot: {
    ...
    paddingVertical: 4, borderRadius: 8, borderWidth: 1, marginLeft: 8,
  },
  ...
  aiInputRow: {
    ...
    paddingHorizontal: 12, paddingVertical: 10, gap: 8, margin: 12,
    marginTop: 8, borderRadius: 16, borderWidth: 1,
  },
  aiInput:   { flex: 1, fontSize: 13, fontFamily: 'Inter_500Medium', minHeight: 36 },
  aiSendBtn: { width: 34, height: 34, borderRadius: 17, alignItems: 'center', justifyContent: 'center' },
});
