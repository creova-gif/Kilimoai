...
export default function InsuranceScreen() {
  ...
  const myPolicies = allPolicies.filter((p) => p.status === 'active' || p.status === 'pending' || p.status === 'claimed');
  ...
  function simulateCameraCapture() {
    ...
    setTimeout(() => {
      ...
      setClaimPhoto('https://images.unsplash.com/photo-1574323347407-f5e1ad6d020b?auto=format&fit=crop&q=80&w=400');
  ...
  function handleClaimSubmit() {
    ...
    const payoutRatio = damageLevel === 'low' ? 0.2 : damageLevel === 'medium' ? 0.5 : damageLevel === 'high' ? 0.8 : 1.0;
  ...
  return (
    <Gate feature="insurance" fallback={<PageScaffold title="Bima" badge="INSURANCE"><AccessDenied /></PageScaffold>}>
      <PageScaffold title="Bima ya Kilimo" subtitle="Crop & livestock protection" badge="INSURANCE HUB">
        ...
        <SectionHeader title={language === 'sw' ? 'Chagua Sera · Available Coverage' : 'Available Coverage'} />
            ...
            <GlassCard key={p.id} style={{ padding: 18 }}>
              <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                ...
                <View style={{ flex: 1, marginLeft: 14 }}>
                  ...
                  <Text style={[s.provider, { color: colors.textMute }]}>{p.provider} · {p.coverage.toUpperCase()}</Text>
                </View>
              </View>
              
              <Text style={{ fontSize: 11.5, fontFamily: 'Inter_500Medium', color: colors.text, marginTop: 10, lineHeight: 17 }}>
                {(p as any).desc || (language === 'sw' ? 'Kinga dhidi ya mabadiliko ya hali ya hewa.' : 'Coverage against climatic anomalies.')}
              </Text>
            ...
            <ScrollView contentContainerStyle={{ padding: 24, paddingBottom: 60 }} showsVerticalScrollIndicator={false}>
              ...
              {enrollStep === 1 && (
                <View style={{ gap: 14 }}>
                  ...
                  <View style={s.inputGroup}>
                    <Text style={[s.inputLabel, { color: colors.textMute }]}>Jina la Mkulima / Farmer Name</Text>
                  ...
                  <View style={s.inputGroup}>
                    <Text style={[s.inputLabel, { color: colors.textMute }]}>Nambari ya Simu / Phone</Text>
                    <View style={[s.inputWrap, { borderColor: colors.border }]}>
                  ...
                  <View style={s.inputGroup}>
                    <Text style={[s.inputLabel, { color: colors.textMute }]}>NIDA / National ID Number</Text>
                    <View style={[s.inputWrap, { borderColor: colors.border }]}>
                  ...
                  <View style={s.inputGroup}>
                    <Text style={[s.inputLabel, { color: colors.textMute }]}>Mkoa na Wilaya / Region</Text>
                    <View style={[s.inputWrap, { borderColor: colors.border }]}>
              ...
              {enrollStep === 2 && (
                <View style={{ gap: 14 }}>
                  ...
                  <View style={s.inputGroup}>
                    <Text style={[s.inputLabel, { color: colors.textMute }]}>Zao Linalokatiwa Bima / Crop to Cover</Text>
                    <TextInput 
                      style={[s.singleTextInput, { color: colors.text, borderColor: colors.border }]} 
                      value={cropCovered} 
                  ...
                  <View style={s.inputGroup}>
                    <Text style={[s.inputLabel, { color: colors.textMute }]}>Ukubwa wa Shamba (Acres) / Farm Size</Text>
                    <TextInput 
                      style={[s.singleTextInput, { color: colors.text, borderColor: colors.border }]} 
                      value={farmAcres} 
                  ...
                  <GlassCard style={{ padding: 14, marginTop: 10, borderColor: colors.primary + '20' }}>
                    ...
                    <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 8 }}>
                      <Text style={{ fontSize: 12, fontFamily: 'Inter_600SemiBold', color: colors.text }}>Max Loss Payout:</Text>
                      <Text style={{ fontSize: 12, fontFamily: 'Inter_800ExtraBold', color: colors.primary }}>
              ...
              {enrollStep === 3 && (
                <View style={{ gap: 16 }}>
                  ...
                  <View style={[s.pnlBox, { borderColor: colors.border }]}>
                    <View style={s.pnlRow}>
                      <Text style={{ fontSize: 12, fontFamily: 'Inter_600SemiBold', color: colors.text }}>Premium Rate:</Text>
                      <Text style={{ fontSize: 13, fontFamily: 'Inter_800ExtraBold', color: colors.text }}>
                    ...
                    <View style={s.pnlRow}>
                      <Text style={{ fontSize: 13, fontFamily: 'Inter_700Bold', color: colors.text }}>Total Due:</Text>
                      <Text style={{ fontSize: 15, fontFamily: 'Inter_800ExtraBold', color: colors.primary }}>
                        TZS {fmt(Math.round((selectedEnrollPolicy?.premiumTZS ?? 0) * 0.7))}
                      </Text>
                    </View>
                  </View>

                  <Text style={[s.inputLabel, { color: colors.textMute, marginBottom: -6 }]}>Chagua Njia ya Malipo / Payment Method</Text>
            ...
            <ScrollView contentContainerStyle={{ padding: 24, paddingBottom: 60 }} showsVerticalScrollIndicator={false}>
              ...
              {claimStep === 1 && (
                <View style={{ gap: 14 }}>
                  ...
                  <View style={s.inputGroup}>
                    <Text style={[s.inputLabel, { color: colors.textMute }]}>Sababu ya Hasara / Reason for Loss</Text>
                    <TextInput 
                  ...
                  <Text style={[s.inputLabel, { color: colors.textMute, marginBottom: -6 }]}>Kiwango cha Uharibifu / Damage Severity</Text>
              ...
              {claimStep === 2 && (
                <View style={{ gap: 14 }}>
                  <Text style={{ fontSize: 13, fontFamily: 'Inter_700Bold', color: colors.text }}>
                    {language === 'sw' ? 'Hatua ya 2: Ushahidi wa Picha' : 'Step 2: Upload Photo Evidence'}
                  </Text>

                  {/* Viewfinder simulator */}
                  {!claimPhoto ? (
                    <View style={[s.viewfinder, { backgroundColor: isDark ? '#111810' : '#f4fbf3', borderColor: colors.border }]}>
                      {isCapturing ? (
                        <View style={{ alignItems: 'center', gap: 10 }}>
                          <ActivityIndicator size="large" color={colors.primary} />
                          <Text style={{ fontSize: 12, fontFamily: 'Inter_600SemiBold', color: colors.textMute }}>
                            {language === 'sw' ? 'Inapakua picha ya ushahidi...' : 'Analyzing crop damage image...'}
                          </Text>
                        </View>
                      ) : (
                        <View style={{ alignItems: 'center', padding: 24, gap: 14 }}>
                          <Camera size={44} color={colors.textMute} style={{ opacity: 0.6 }} />
                          <Text style={{ fontSize: 12, fontFamily: 'Inter_600SemiBold', color: colors.textMute, textAlign: 'center' }}>
                            {language === 'sw' 
              ...
              {claimStep === 3 && (
                <View style={{ gap: 16 }}>
                  <Text style={{ fontSize: 13, fontFamily: 'Inter_700Bold', color: colors.text }}>
                    {language === 'sw' ? 'Hatua ya 3: Uhakiki na Kutuma' : 'Step 3: Review & Submit'}
                  </Text>

                  <View style={[s.pnlBox, { borderColor: colors.border }]}>
                    <View style={s.pnlRow}>
                      <Text style={{ fontSize: 12, fontFamily: 'Inter_600SemiBold', color: colors.textMute }}>Sera / Policy:</Text>
                      <Text style={{ fontSize: 12, fontFamily: 'Inter_700Bold', color: colors.text }}>
                        {selectedClaimPolicy?.product}
                      </Text>
                    </View>
                    <View style={s.pnlRow}>
                      <Text style={{ fontSize: 12, fontFamily: 'Inter_600SemiBold', color: colors.textMute }}>Mtoa Huduma / Provider:</Text>
                      <Text style={{ fontSize: 12, fontFamily: 'Inter_700Bold', color: colors.text }}>
                        {selectedClaimPolicy?.provider}
                      </Text>
                    </View>
                    <View style={s.pnlRow}>
                      <Text style={{ fontSize: 12, fontFamily: 'Inter_600SemiBold', color: colors.textMute }}>Kiwango cha Hasara:</Text>
                      <Text style={{ fontSize: 12, fontFamily: 'Inter_800ExtraBold', color: '#ef4444', textTransform: 'uppercase' }}>
                        {damageLevel}
                      </Text>
                    </View>
                    <View style={{ height: 1, backgroundColor: colors.border, marginVertical: 4 }} />
                    <View style={s.pnlRow}>
                      <Text style={{ fontSize: 13, fontFamily: 'Inter_700Bold', color: colors.text }}>Fidia Inayotarajiwa:</Text>
                      <Text style={{ fontSize: 16, fontFamily: 'Inter_800ExtraBold', color: '#ef4444' }}>
                        TZS {fmt(Math.round((selectedClaimPolicy?.payoutMaxTZS ?? 0) * (damageLevel === 'low' ? 0.2 : damageLevel === 'medium' ? 0.5 : damageLevel === 'high' ? 0.8 : 1.0)))}
                      </Text>
                    </View>
                  </View>
                    ...
                    onPress={() => {
                      if (claimStep === 2 && !claimPhoto) {
                        Alert.alert(
                          ...
                          language === 'sw' ? 'Tafadhali piga picha kwanza.' : 'Please capture photo evidence before continuing.'
                        );
                        return;
                      }
...
function AccessDenied() {
  ...
  return (
    <EmptyState
      ...
      title="Haipatikani"
      body={language === 'sw' ? "Bima haipatikani kwa jukumu lako." : "Insurance features are not accessible for your role."}
    />
  );
}
...
const s = StyleSheet.create({
  ...
  textarea: { borderWidth: 1, borderRadius: 12, padding: 12, minHeight: 90, textAlignVertical: 'top', fontSize: 13, fontFamily: 'Inter_500Medium' },
  ...
  modalPriBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', height: 48, borderRadius: 14, gap: 6 },
  modalPriBtnText: { color: '#000', fontSize: 13.5, fontFamily: 'Inter_700Bold' },
  modalSecBtn: { borderWidth: 1, borderRadius: 14, height: 48, paddingHorizontal: 20, justifyContent: 'center', alignItems: 'center' },
  ...
  photoSuccessOverlay: { position: 'absolute', top: 12, right: 12, width: 32, height: 32, borderRadius: 16, backgroundColor: '#fff', justifyContent: 'center', alignItems: 'center' },
  evidenceBadge: { flexDirection: 'row', alignItems: 'center', gap: 6, borderWidth: 1, borderRadius: 14, justifyContent: 'center', height: 48 },
  thumbnail: { width: 56, height: 56, borderRadius: 10 },
  agreeBox: { padding: 12, backgroundColor: 'rgba(0,0,0,0.02)', borderRadius: 10, borderWidth: 0.5, borderColor: '#ccc' },
});
