...
export function QuickAccessCards({
  ...
  return (
    <div className="grid gap-3 md:gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
      ...
      >
        <Card 
          className="border-2 border-[#2E7D32]/20 bg-[#2E7D32]/5 cursor-pointer hover:shadow-lg transition-all group"
          onClick={onTipClick}
        >
          <CardContent className="p-4">
            <div className="flex items-start justify-between mb-3">
              <div className="p-2 bg-[#2E7D32]/10 rounded-lg group-hover:bg-[#2E7D32]/20 transition-colors">
                <Sparkles className="h-5 w-5 text-[#2E7D32]" />
              </div>
              <ChevronRight className="h-5 w-5 text-[#2E7D32] group-hover:text-[#2E7D32] group-hover:translate-x-1 transition-all" />
            </div>
            <h3 className="font-bold text-sm mb-2 text-[#2E7D32]">Today's Tip</h3>
            <p className="text-xs text-[#2E7D32] line-clamp-2 leading-relaxed">
              {todayTip}
            </p>
            <div className="mt-3 flex items-center gap-1 text-xs text-[#2E7D32] font-medium">
      ...
      >
        ...
        >
          <CardContent className="p-4">
            ...
            <h3 className="font-bold text-sm mb-2 text-[#2E7D32]">Market Price</h3>
            <div className="mb-1">
              <p className="text-xs text-[#2E7D32] font-medium">{marketTrend.crop}</p>
              <div className="flex items-baseline gap-2">
                <p className="text-lg font-bold text-[#2E7D32]">
                  TZS {marketTrend.price.toLocaleString()}
                </p>
                <span className={`text-xs font-medium ${
                  marketTrend.change >= 0 ? "text-[#2E7D32]" : "text-red-600"
                }`}>
