...
interface VisualCropPlannerProps {
  userId: string;
  language?: "en" | "sw";
...
export function VisualCropPlanner({
  userId,
  language = "en",
  totalFarmSize = 100
}: VisualCropPlannerProps) {
  ...
  const [plantings, setPlantings] = useState<Planting[]>([
    {
      ...
      startWeek: getWeekNumber(new Date("2026-03-01")),
      durationWeeks: Math.ceil(120 / 7)
    },
    {
      id: "p2",
      cropId: "2",
      ...
      startWeek: getWeekNumber(new Date("2026-03-15")),
      durationWeeks: Math.ceil(90 / 7)
    }
  ]);
  ...
  const handleAddPlanting = async () => {
    ...
    const harvestDate = calculateHarvestDate(
      newPlanting.plantingDate, 
      crop.growthCycle
    );

    // Calculate revenue
    const totalYield = newPlanting.acres * template.expectedYield;
    const revenue = totalYield * 1000 * crop.marketPrice; // tons to kg
    ...
    // Auto-generate tasks
    await autoGenerateTasks(planting, template, crop);
  ...
  const renderFieldView = (months: any[]) => {
    return fields.map(field => {
      ...
      return (
        <div key={field.id} className="flex border-b border-gray-200 hover:bg-gray-50">
          <div className="w-40 px-4 py-3 border-r border-gray-200">
            <div className="flex items-center gap-2">
  ...
  const renderPlantingBlock = (planting: Planting, months: any[]) => {
    ...
    return (
      <div
        ...
        draggable
        onDragStart={() => setDraggedPlanting(planting.id)}
            ...
            <div className="flex items-start justify-between mb-3">
              ...
              <div className="flex gap-1">
                ...
                <Button
                  variant="ghost"
                  size="sm"
                  onClick={() => handleDeletePlanting(selectedPlanting.id)}
