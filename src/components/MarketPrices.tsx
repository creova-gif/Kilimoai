...
export function MarketPrices({ region, onNavigate }: MarketPricesProps) {
  ...
  const getTrendColor = (trend: string) => {
    switch (trend) {
      case "up": return "text-[#2E7D32] bg-[#2E7D32]/5 border-[#2E7D32]/20";
  ...
  const getForecastBadge = (forecast: string) => {
    switch (forecast) {
      case "rising":
        return <Badge className="bg-[#2E7D32]/5 text-[#2E7D32] flex items-center gap-1"><TrendingUp className="h-3 w-3" />Rising</Badge>;
      case "falling":
  ...
  return (
    <div className="space-y-4 md:space-y-6 pb-6">
      {/* Header */}
      <div className="relative overflow-hidden bg-[#2E7D32] rounded-2xl md:rounded-3xl p-4 md:p-6 text-white">
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="grid grid-cols-2 gap-2">
          <Button 
            ...
            className="gap-2 h-11 rounded-xl border-2 hover:border-[#2E7D32] hover:bg-[#2E7D32]/5 transition-all"
          >
